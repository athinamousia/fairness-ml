...
def run_grid_search(model, params, X_train, y_train, scorer, weights=None, njobs=4):
    ...
    grid = GridSearchCV(
        model, params, scoring=scorer, cv=5, n_jobs=njobs
    )
...
def prepare_data(
    ...
    X_train, X_test, y_train, y_test, idx_train, idx_test = train_test_split(
        X, y, range(len(df)), train_size=train_ratio, random_state=random_state
    )
...
def build_metrics_list(
    ...
    eval_df = pd.DataFrame({
        ...
        "Algorithm": [alg_name] * len(eval_metrics) + ["XGBoost"] * len(eval_metrics),
    })
    result.append(eval_df)

    # ---- Fairness metrics per protected attr
    for attr in protected_attrs:
        mit_vals = mitigation_df[mitigation_df["Protected Attribute"] == attr][fairness_metrics].values[0]
        base_vals = baseline_df[baseline_df["Protected Attribute"] == attr][fairness_metrics].values[0]
